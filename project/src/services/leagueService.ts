...
export class LeagueService {
  ...
  static async getAvailableRewardPlayers(
    serverId: string
    ...
    Object.entries(teamsData.teams).forEach(
  ...
  static async startLeague(leagueId: string): Promise<void> {
    ...
    console.log(
      `Generated ${fixtures.length} fixtures for ${league.clubs.length} clubs`
    );
  ...
  /**
   * FIXED: Generate fixtures with proper round-robin scheduling
   * Handles odd/even number of clubs, assigns multiple matches per matchDay
   * Uses "bye" for clubs that don't play in a round (odd numbers)
   */
  private static async generateFixtures(
    league: League
    ...
    console.log(`Generating fixtures for ${numClubs} clubs`);
    ...
    const clubDetails = await Promise.all(
      clubs.map(async (clubId) => {
        ...
        const clubsSnapshot = await getDocs(
          query(clubsRef, where("id", "==", clubId))
        );
        return clubsSnapshot.docs[0]?.data() as Club;
      })
    );

    // For round-robin scheduling with odd numbers, we add a "dummy" club
    // This ensures proper rotation and bye weeks
    const isOddNumber = numClubs % 2 === 1;
    const totalTeams = isOddNumber ? numClubs + 1 : numClubs;
    const teamsForScheduling = [...clubs];

    if (isOddNumber) {
      teamsForScheduling.push("BYE"); // Dummy team for bye week
    }
    ...
    for (let round = 0; round < 2; round++) {
      console.log(`Generating round ${round + 1} fixtures`);

      // Round-robin algorithm: Each team plays every other team once per round
      for (let week = 0; week < totalTeams - 1; week++) {
        ...
        for (let match = 0; match < totalTeams / 2; match++) {
          const home = (week + match) % (totalTeams - 1);
          const away = (totalTeams - 1 - match + week) % (totalTeams - 1);

          // If one team is the last team, swap with the "fixed" team
          const homeTeam = home === totalTeams - 1 ? totalTeams - 1 : home;
          const awayTeam = away === totalTeams - 1 ? totalTeams - 1 : away;

          const homeClubId = teamsForScheduling[homeTeam];
          const awayClubId = teamsForScheduling[awayTeam];

          // Skip if either team is "BYE" (bye week for odd-numbered leagues)
          if (homeClubId === "BYE" || awayClubId === "BYE") {
            continue;
          }

          // For round 2, swap home and away
          const finalHomeId = round === 0 ? homeClubId : awayClubId;
          const finalAwayId = round === 0 ? awayClubId : homeClubId;

          const homeClub = clubDetails.find((c) => c.id === finalHomeId);
          const awayClub = clubDetails.find((c) => c.id === finalAwayId);
        ...
        // Add all fixtures for this week
        fixtures.push(...weekFixtures);

        // Only increment matchDay if we have fixtures (skip bye weeks with no matches)
        if (weekFixtures.length > 0) {
          console.log(`MatchDay ${matchDay}: ${weekFixtures.length} fixtures`);
          matchDay++;
        }
      }
    }
  ...
  static async playMatch(
    ...
    if (fixture.status !== "available")
      throw new Error("Match not available or already played");

    // Check if match date has arrived
    const now = new Date();
    const matchDate =
      fixture.scheduledDate &&
      typeof fixture.scheduledDate === "object" &&
    ...
    try {
      ...
      const simulationResult = await MatchSimulationService.simulateMatch(
        homeClub,
        awayClub,
        true
      );
      console.log(
        `Match result: ${homeClub.clubName} ${simulationResult.homeScore} - ${simulationResult.awayScore} ${awayClub.clubName}`
      );
      ...
      const matchResult: LeagueMatch = {
        ...
        playedAt: new Date(),
        isForfeited: false,
        goalscorers: simulationResult.events
        ...
        assists: simulationResult.events
          .filter((e) => e.type === "goal" && e.assistPlayerId)
          .map((e) => ({
        ...
        cards: simulationResult.events
          .filter((e) => e.type === "yellow_card" || e.type === "red_card")
          .map((e) => ({
            playerId: e.playerId!,
            playerName: e.playerName!,
            type:
              e.type === "yellow_card" ? ("yellow" as const) : ("red" as const),
            minute: e.minute,
            isHome: e.isHome,
          })),
        commentary: [
  ...
  private static async markFixtureAsPlaying(
    leagueId: string,
    fixtureId: string
  ): Promise<void> {
    const leagueDoc = await getDoc(doc(db, "leagues", leagueId));
  ...
  private static async resetFixtureStatus(
    leagueId: string,
    fixtureId: string
  ): Promise<void> {
    const leagueDoc = await getDoc(doc(db, "leagues", leagueId));
   ...
   */
  private static async updatePlayerStaminaAndCards(
    homeClub: Club,
    awayClub: Club,
    match: LeagueMatch
  ): Promise<void> {
    ...
    const updatedHomePlayers = homeClub.players.map((player) => {
      ...
      if (player.squadPosition === "starter") {
        ...
        console.log(
          `${player.name} (starter): stamina ${currentStamina}% -> ${updatedPlayer.staminaPct}%`
        );
      } else if (
        player.squadPosition === "substitute" ||
        player.squadPosition === "reserve"
      ) {
        updatedPlayer.staminaPct = Math.min(100, currentStamina + 10);
        console.log(
          `${player.name} (${player.squadPosition}): stamina ${currentStamina}% -> ${updatedPlayer.staminaPct}%`
        );
      }

      // Update cards from match
      const playerCards = match.cards.filter(
        (c) => c.playerId === player.id && c.isHome
      );
      ...
      playerCards.forEach((card) => {
        if (card.type === "yellow") {
          ...
          console.log(
            `${player.name}: yellow card ${previousYellow} -> ${updatedPlayer.yellowCards}`
          );
   ...
   */
  private static async updateLeagueWithResult(
    leagueId: string,
    match: LeagueMatch
  ): Promise<void> {
    ...
    const updatedLeaderboard = this.updateLeaderboard(
      league.leaderboard,
      match
    );
    console.log(
      "Updated leaderboard:",
      updatedLeaderboard.map((t) => `${t.clubName}: ${t.points}pts`)
    );
    ...
    const currentDayFixtures = updatedFixtures.filter(
      (f) => f.matchDay === league.currentMatchDay
    );
    const allCurrentDayPlayed = currentDayFixtures.every(
      (f) => f.status === "played" || f.status === "forfeited"
    );
    ...
    console.log(
      `Current matchDay ${league.currentMatchDay}: ${currentDayFixtures.length} fixtures, all played: ${allCurrentDayPlayed}`
    );
    ...
    if (allCurrentDayPlayed) {
      ...
      console.log(
        `Made ${nextDayFixtures} fixtures available for matchDay ${nextMatchDay}`
      );

      // Check if league is finished
      const totalFixtures = updatedFixtures.length;
      const playedFixtures = updatedFixtures.filter(
        (f) => f.status === "played"
      ).length;

      console.log(
        `League progress: ${playedFixtures}/${totalFixtures} fixtures played`
      );
   ...
   */
  private static updateLeaderboard(
    ...
    const updatedLeaderboard = leaderboard.map((team) => {
      if (team.clubId === match.homeClubId) {
        ...
        const homeYellowCards =
          match.cards?.filter((c) => c.isHome && c.type === "yellow").length ||
          0;
        const homeRedCards =
          match.cards?.filter((c) => c.isHome && c.type === "red").length || 0;
        ...
        const updated = {
          ...
          drawn:
            match.homeScore === match.awayScore ? team.drawn + 1 : team.drawn,
          ...
          goalDifference:
            team.goalDifference + (match.homeScore - match.awayScore),
        ...
        console.log(
          `${team.clubName}: +${points} points, GD: ${
            match.homeScore - match.awayScore
        ...
        const updated = {
          ...
          drawn:
            match.awayScore === match.homeScore ? team.drawn + 1 : team.drawn,
          ...
          goalDifference:
            team.goalDifference + (match.awayScore - match.homeScore),
          points: team.points + points,
   ...
   */
  private static updateTopScorers(
    ...
    match.goalscorers.forEach((goal) => {
      const existingScorer = updatedScorers.find(
        (s) => s.playerId === goal.playerId
      );
      if (existingScorer) {
        existingScorer.goals++;
        console.log(
          `${existingScorer.playerName}: ${existingScorer.goals} goals`
        );
      } else {
        const newScorer = {
   ...
   */
  private static updateTopAssists(
    ...
    match.assists.forEach((assist) => {
      const existingAssister = updatedAssists.find(
        (a) => a.playerId === assist.playerId
      );
      if (existingAssister) {
        existingAssister.assists++;
        console.log(
          `${existingAssister.playerName}: ${existingAssister.assists} assists`
        );
      } else {
        const newAssister = {
  ...
  static async terminateLeague(leagueId: string): Promise<void> {
    ...
    if (league.playerReward && league.leaderboard.length > 0) {
      await this.awardPlayerReward(
        league.leaderboard[0].clubId,
        league.playerReward
      );
    }
  ...
  private static async awardPlayerReward(
    clubId: string,
    playerReward: FirebasePlayer
  ): Promise<void> {
    ...
    if (club.players.length >= 23) {
      console.warn(
        "Club already has maximum players, cannot add reward player"
      );
      return;
    }
  ...
  static async processAutoForfeits(leagueId: string): Promise<number> {
    ...
    for (const fixture of league.fixtures) {
      ...
      if (today >= dayAfterMatch && fixture.status === "available") {
        console.log(
          `Auto-forfeiting match: ${fixture.homeClubName} vs ${
            fixture.awayClubName
          } (scheduled for ${matchDate.toDateString()})`
        );
    ...
    if (forfeitCount > 0) {
      ...
      const allCurrentDayComplete = currentDayFixtures.every(
        (f) => f.status === "played" || f.status === "forfeited"
      );
      ...
      if (allCurrentDayComplete) {
        nextMatchDay++;
        console.log(
          `Advancing to matchDay ${nextMatchDay} after auto-forfeits`
        );
        ...
        // Check if league is finished
        const totalFixtures = updatedFixtures.length;
        const completedFixtures = updatedFixtures.filter(
          (f) => f.status === "played" || f.status === "forfeited"
        ).length;
   ...
   */
  static async processAllLeagueForfeits(): Promise<{
    ...
    for (const doc of snapshot.docs) {
      ...
      try {
        ...
        console.error(
          `Error processing forfeits for league ${league.id}:`,
          error
        );
  ...
  static async getLeagueStatistics(leagueId: string): Promise<{
    ...
    const forfeitedFixtures = league.fixtures.filter(
      (f) => f.status === "forfeited"
    ).length;
