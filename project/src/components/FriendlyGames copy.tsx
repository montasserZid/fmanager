...
export function FriendlyGames({ club, onUpdateClub }: FriendlyGamesProps) {
  ...
  const handleAcceptInvite = async (invite: FriendlyInvite) => {
    ...
    try {
      ...
      // Update stamina for all players who played
      const staminaUpdates = matchResult.staminaImpact.map(impact => 
        FriendlyService.updatePlayerStamina(impact.playerId, impact.staminaAfter)
      );
      await Promise.all(staminaUpdates);
      
      // Update club players with new stamina values
      const updatedPlayers = club.players.map(player => {
        const impact = matchResult.staminaImpact.find(i => i.playerId === player.id);
        if (impact) {
          return { ...player, staminaPct: impact.staminaAfter };
        }
        return player;
      });
      ...
      await Promise.all([
        ...
        FriendlyService.updateLastFriendlyDate(invite.fromUserId)
      ]);
      
      // Accept invite and clean up
      await FriendlyService.respondToInvite(invite.id, true);
